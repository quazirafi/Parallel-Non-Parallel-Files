...
// Write dataset to file dataset.bin(εγραφη δεδομενων σε αρχειο)
...
void cube(float *X, int N){//Παραγωγή τυχαίων αριθμών και εισαγωγή σε πίνακα

  srand(time(NULL));//Αρχικοποίηση γεννήτριας τυχαίων αριθμών
			dummyMethod1();
#pragma omp parallel for shared(N) schedule(static)                /*AAAAAAAAAAAAAAAAAAAAAAAAA*/
  for(int i=0; i<N; i++){
    for(int j=0; j<DIM; j++){
      X[i*DIM + j] = (float)rand() / (float) RAND_MAX;//Παραγωγή τυχαίων αριθμών και εισαγωγή στον x[]
    }
  }
			dummyMethod2();
...
#pragma omp parallel for shared(N) schedule(static)                /*AAAAAAAAAAAAAAAAAAAAAAAAA*/
  for(int i=0; i<N; i++){
    ...
    X[i*DIM + 1] = ((float)rand() / (float) RAND_MAX) *sqrt(1 - pow(X[i*DIM], 2));

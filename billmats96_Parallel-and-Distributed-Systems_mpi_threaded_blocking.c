...
int main(int argc, char** argv)
{
	...
	double time,timecalc=0,timecomm,tcom,telse;
	MPI_Status status;   // required variable for Recv routine
	...
	else
	{
		...
		//fp=fopen("/home/linuxbill/Desktop/parallel_2/MYoutputf.bin","rb");
	...
	//Rec_value is the block number which is being passed around by the processes.Has to be initialized here for each process.
	 ...
	 if (rank==MASTER)
	{
	 ...
	 while(value>0){
		 MPI_Barrier (MPI_COMM_WORLD);
		if(rank==MASTER) tcom-=MPI_Wtime();

	  //blocking receives and sends for neighbours
		 if(rank!=size-1)
	   {
		  MPI_Send(&rec_value, 1, MPI_INT, next, tag1, MPI_COMM_WORLD);
		  MPI_Send(dynarr_2,chunk,MPI_FLOAT,next,tag2,MPI_COMM_WORLD);

	   }
	   if(rank!=MASTER)
	   {
		  MPI_Recv(receivebuf,chunk,MPI_FLOAT,prev,tag2,MPI_COMM_WORLD,&status);
	      MPI_Recv(&store_value, 1, MPI_INT, prev, tag1, MPI_COMM_WORLD,&status);
	   }
	   MPI_Barrier (MPI_COMM_WORLD); //have to wait for all the processes to execute the section above
	  //execute the last send receive separately.
	   if(rank==size-1)
	   {
		  MPI_Send(&rec_value, 1, MPI_INT, next, tag1, MPI_COMM_WORLD);
		  MPI_Send(dynarr_2,chunk,MPI_FLOAT,next,tag2,MPI_COMM_WORLD);
	   ...
	   if(rank==MASTER)
	   {
		...
		//Synchronize and take into account the calculations below.
	   MPI_Barrier (MPI_COMM_WORLD);
	   if(rank==MASTER) { timecalc-=MPI_Wtime(); tcom+=MPI_Wtime(); }
			dummyMethod3();
		for(int i=0; i<chunk; i++)
	     {
	   ...
	   value--;
	   MPI_Barrier (MPI_COMM_WORLD);
	   if(rank==MASTER) timecalc+=MPI_Wtime();
	}

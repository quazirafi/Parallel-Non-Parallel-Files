...
int main(int argc, char** argv){
 ...
 // Time counting variables //Μεταβλητές για μέτρηση χρονου
...
/*--------------------------------------------του προγράμματος-----------------------------------------------------*/
  ...
  numthreads=atoi(argv[6]); //Ορισμός του αριθμού των threads
/*-----------------------------------------------------------------------------------------------------------------*/
/*-----------------------------------------------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------------------------------*/
   ...
   // Ορίζουμε τον αριθμό των threads
   ...
   // initialize the index
	dummyMethod1();
 #pragma omp parallel for shared(index,N) schedule(static)
 for(int i=0; i<N; i++){        
      index[i] = i;
      }
	dummyMethod2();
...
/*C*/   truncated_radix_sort(morton_codes, sorted_morton_codes,  // Truncated msd radix sort
			 permutation_vector, 
...
// double endtime=omp_get_wtime();
 // printf("The total time of the program      : %fs\n",endtime - starttime );

...
main (int argc, char *argv[])
{
  ...
  // each item is touched by a different NUMA node
			dummyMethod1();
#pragma omp parallel for shared(wu)
  for (int i = 0; i < n; i++)
      wu[i] = 0;
			dummyMethod2();
  ...
  // make sure following shared variables are touched
  // by different NUMA nodes
  // shared (bins, space_left_on_bin, items, item_in_bin)

  // initialize bin size
  // initially all bins are empty
  bins = (int *) malloc (sizeof (int) * max_num_bins * max_items_per_bin);
  // each bin is touched by a different NUMA node
			dummyMethod1();
#pragma omp parallel for shared(bins) schedule(dynamic)
  for (int i = 0; i < max_num_bins; i++)
  {
      for (int j = 0; j < max_items_per_bin; j++)
	  bins[i * max_items_per_bin + j] = 0;
  }
			dummyMethod2();
  ...
  // each item is touched by a different NUMA node
			dummyMethod1();
#pragma omp parallel for shared(item_in_bin) schedule(dynamic)
  ...
  space_left_on_bin = (int *) malloc (sizeof (int) * max_num_bins);
			dummyMethod1();
#pragma omp parallel for private(C) shared(space_left_on_bin) schedule(dynamic)
  for (int i = 0; i < max_num_bins; i++)
  ...
  for (int i = 0; i < max_num_bins; i++)
    {
      if (space_left_on_bin[i] >= 0)	// no need to show empty bins
	{
	  printf ("\n bin %d: ", i + 1);
	  for (int j = 0; j < max_items_per_bin; j++)

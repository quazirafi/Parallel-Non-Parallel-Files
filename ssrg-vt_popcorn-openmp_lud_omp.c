...
#pragma omp threadprivate(temp, temp_top, temp_left, sum)

// implements block LU factorization 
void lud_omp(float *a, int size)
{
    int offset = 0, chunk_idx, size_inter, chunks_in_inter_row, chunks_per_inter;
...
#ifdef OMP_OFFLOAD
{
...
#ifdef _OPTIMIZED
    #pragma omp parallel default(none) private(offset, chunk_idx) \
       shared(size, chunks_per_inter, chunks_in_inter_row, a)
#endif
#endif
    for (offset = 0; offset < size - BS ; offset += BS)
    {
        ...
        #pragma omp parallel for default(none) \
          private(chunk_idx) shared(size, chunks_per_inter, chunks_in_inter_row, offset, a) schedule(runtime)
#endif
...
#else
															dummyMethod1();
#pragma omp parallel for schedule(SCHEDULE) default(none) \

...
int minChunk = 0; /* no tuning for num_threads, meaning we will run the program with fixed number of threads */
int maxNumThreads = 0;
...
int main(int argc, char *argv[]) {
    ...
    if (argc == 3) {
        ...
        MEDIUM = strtoll(argv[3], NULL, 10);
        OUTERLARGE = strtoll(argv[4], NULL, 10);
        minChunk = strtoll(argv[5], NULL, 10);
        useBuiltInData = false;
    } else if (argc == 7) {
        m = strtoll(argv[1], NULL, 10);
        n = strtoll(argv[2], NULL, 10);
        ...
        OUTERLARGE = strtoll(argv[4], NULL, 10);
        minChunk = strtoll(argv[5], NULL, 10);
        maxNumThreads = strtoll(argv[6], NULL, 10);
        useBuiltInData = false;
    }
...
*/
    {
        ...
        for (i = 1; i <= nDiag; ++i) // start from 1 since 0 is the boundary padding
        {
            ...
            if (nDiag < OUTERLARGE) { // omp cpu version: medium to large: medium data set
                iterationTime = omp_get_wtime();
                int numThreads;
                if (minChunk == 0) numThreads = maxNumThreads;
                else numThreads = min(nEle/minChunk + 1, maxNumThreads);
																															dummyMethod1();
#pragma omp parallel for private(j) shared (nEle, si, sj, H, P) num_threads(numThreads)

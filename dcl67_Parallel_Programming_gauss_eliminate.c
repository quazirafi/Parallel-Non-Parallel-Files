...
//void gauss_eliminate_using_openmp(const Matrix, const Matrix, unsigned int);
...
main(int argc, char** argv) {
	...
	gauss_eliminate_using_openmp(A, U, A.num_rows);
...
void 
gauss_eliminate_using_openmp(Matrix A, Matrix U, unsigned int num_elements)                  /* Write code to perform gaussian elimination using OpenMP. */
//gauss_eliminate_using_openmp(const float* A, float* U, unsigned int num_elements)     /* Write code to perform gaussian elimination using OpenMP. */ 

{
	unsigned int i, j, k;
							dummyMethod1();
	#pragma omp parallel for default(none) private(i, j, k) shared(A, U, num_elements) num_threads(NUM_THREADS)

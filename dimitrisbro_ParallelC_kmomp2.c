/*
gcc gcc kmomp2.c -lm -O3 -fopenmp
time ./a.out
...
float estimateClass( void )
{
    ...
    float mdist=0;
							...
							dummyMethod1();
    #pragma omp parallel for reduction(+:mdist)//addition of reduction 
    for(int i=0;i<N;i++){
        int th=-1;

        float min=350000;
        #pragma omp parallel for reduction(min:min)//addition of reduction 
        for (int j=0;j<Nc;j++){
            float dist=0;
            for (int k=0;k<Nv;k++){

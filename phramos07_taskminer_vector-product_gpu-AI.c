...
{
    int i;
    #pragma acc data copy(B[0:1000],A[0:1000])
    #pragma acc kernels
...
void product_GPU(float *A, float *B, float *C)
{
	...
	#pragma omp parallel for
        #pragma acc data copy(C[0:1000],B[0:1000],A[0:1000])
        #pragma acc kernels
        #pragma acc loop independent
        for (i = 0; i < SIZE; ++i)
	{
...
void product_CPU(float *A, float *B, float *C)
{
    ...
    #pragma acc data copy(B[0:1000],C[0:1000],A[0:1000])
    #pragma acc kernels
    #pragma acc loop independent
							dummyMethod3();
    for (i = 0; i < SIZE; ++i)
...
void compareResults(float *A, float *A_outputFromGpu)
{
    ...
    #pragma acc data copy(A_outputFromGpu[0:1000],A[0:1000])
    #pragma acc kernels
    #pragma acc loop independent
							dummyMethod3();
    for (i=0; i < SIZE; i++) 

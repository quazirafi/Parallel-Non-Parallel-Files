...
#pragma omp for
	...
	//#pragma omp parallel for private(i) //Cannot
							dummyMethod3();
...
/* Search for all the elements that contain the source node */
  ...
  if (Src.sourcenode != 0) {
//#pragma omp parallel for private(i,j,k,cor,vertices)
							dummyMethod3();
...
/* Simulation */
//#pragma omp parallel for private(i,j,k,Me,Ce,v,Ke,cor,verticesonbnd,bv,vertices,alpha,Mexv, Cexv,kk,ii,jj)
			dummyMethod3();
  for (i = 0; i < ARCHelems; i++) {
//    #pragma omp parallel for private(j,k)
    for (j = 0; j < 12; j++) {
...
//#pragma omp parallel for private(j,k,kk,ii,jj)
    for (j = 0; j < 4; j++) {
...
/* Time integration loop */
  ...
  fprintf(stderr, "\n");
//#pragma omp parallel for  private(iter,i,j)
...
struct properties *prop;
{
  ...
  shape_ders(ds);
//#pragma omp parallel for private(i,j,m,sum)
  for (i = 0; i < 3; i++)
    for (j = 0; j < 3; j++) {
      sum[0] = 0.0;
      for (m = 0; m < 4; m++)
...
//#pragma omp parallel for private(m,i,j,sum)
  for (m = 0; m < 4; m++) {

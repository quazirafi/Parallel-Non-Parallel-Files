...
int main(int argc, char* argv[]) {
  ...
  int thread_count;
  xomp_acc_init();
  int calculation_kind;
  if (argc == 3)
  {
    calculation_kind = atoi(argv[1]);
    m = strtoll(argv[2], NULL, 10);
    n = strtoll(argv[2], NULL, 10) + 1;
...
//  #pragma omp parallel default(none) shared(H, P, maxPos, nDiag, j) private(i)
        ...
        //if (nEle< MEDIUM)
	{
          ...
          {  // going upwards : anti-diagnol direction
            ...
            similarityScore2(ai, aj, H, P, maxPos_ptr); // a specialized version without a critical section used inside
          }
	}
	else if (calculation_kind == 1) // omp cpu version: medium to large: medium data set
   ...
   #pragma omp parallel for private(j) shared (nEle, si, sj, H, P, maxPos_ptr) 
	  ...
	  {  // going upwards : anti-diagnol direction
	    long long int ai = si - j ; // going up vertically
	    long long int aj = sj + j;  //  going right in horizontal
	    similarityScore(ai, aj, H, P, maxPos_ptr); // a critical section is used inside
	  }
					dummyMethod2();
	}

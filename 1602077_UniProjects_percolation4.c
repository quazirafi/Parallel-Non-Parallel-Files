...
#pragma omp parallel default(shared) private(myseed)
  ...
  /--------------------------------*/
			dummyMethod1();
  #pragma omp parallel for private(i, ip, igraph, j, Ncon, Lcon, Pcon, xi,avnclus,avlclus) shared(nclus, lclus)
  for (ip=0;ip<Np;ip++) {
    ...
    / Loop over graphs  /
    /------------------*/
    #pragma omp parallel for private(igraph, i, j, xi, Ncon, Lcon) shared(nclus, lclus) reduction(+: avnclus, avlclus)

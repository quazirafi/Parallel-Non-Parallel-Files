...
int main ()   
{   
   long i;  long Ncirc = 0, chunk =1000;   
   double pi, x, y, test;   
   ...
   seed(-r, r);  // The circle and square are centered at the origin   
					...
					dummyMethod1();
   #pragma omp parallel for default(shared) private(i, x, y, test) \
   schedule(static, chunk) reduction(+:Ncirc)
   for(i=0;i<num_trials; i++)   
   {
	#pragma omp parallel shared(x,y) private(random_last) 
	{  
	#pragma omp sections
		{ 
		  #pragma omp section

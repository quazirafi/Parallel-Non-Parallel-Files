...
/* Function to apply the filter with the filter list in the outside loop */
void parallelFilterFirstOP ( int data_len, unsigned int* input_array, unsigned int* output_array, int filter_len, unsigned int* filter_list )
{
  /* Variables for timing */
  struct timeval ta, tb, tresult;
...
//  input_array = (unsigned int*) posix_memalign ( DATA_LEN * sizeof(unsigned int), 4096);
  ...
  /* Execute at a variety of filter lengths */
   //serialFilterFirst ( DATA_LEN, input_array, serial_array, filter_len, filter_list );
   // memset ( output_array, 0, DATA_LEN );

    omp_set_num_threads(8);
    parallelFilterFirstOP ( DATA_LEN, input_array, output_array, filter_len, filter_list );

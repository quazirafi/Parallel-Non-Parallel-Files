...
int main(int argc, char *argv[])
{
  HashFunction hf[] = { RSHash, JSHash, BKDRHash, ELFHash, SDBMHash,
	...
	dummyMethod1();
	#pragma omp parallel for  private(i,j, hash) schedule(guided)  collapse(2)          //shared(wl_size, num_hf, wl)
	for (i = 0; i < wl_size; i++) {
		for (j = 0; j < num_hf; j++) {
	...
	misspelled = 0;
	dummyMethod1();
	#pragma omp parallel for  private (j, hash) schedule(guided) //        shared(word, bv_size)
	for (j = 0; j < num_hf; j++) {
		hash = hf[j] (word);
		hash %= bv_size;
		if (bv[hash] == 0)

...
#pragma omp parallel default(none) shared(length_of_array, a, b, sum_of_a, sum_of_b)
{
...
dummyMethod4();
#pragma omp for private(i) reduction(+:sum_of_a, sum_of_b) 
			dummyMethod3();
  for (i = 0; i < length_of_array; i++){
    sum_of_a += a[i];
    sum_of_b += b[i];
...
#pragma omp parallel for default(none) shared(length_of_array, a, mean_of_a, b, mean_of_b) private(i) reduction(+:sum_of_sqrd_a_a_bar, sum_of_sqrd_b_b_bar, sum_of_a_times_b)
  ...
  printf("The time to initialise arrays and sum was %.5f ms \n", ((sum_end-init_start) *1000));

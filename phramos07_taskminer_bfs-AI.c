...
void BFSGraph(int argc, char** argv, int no_of_nodes, int edge_list_size);
...
int main( int argc, char** argv) 
{
	...
	BFSGraph( argc, argv, no_of_nodes, edge_list_size);
}
...
void compareResults(int* h_cost, int* h_cost_gpu, int no_of_nodes) {
  ...
  long long int AI1[4];
  AI1[0] = no_of_nodes > 0;
  AI1[1] = (AI1[0] ? no_of_nodes : 0);
  AI1[2] = 4 * AI1[1];
  AI1[3] = AI1[2] / 4;
  #pragma acc data copy(h_cost[0:AI1[3]],h_cost_gpu[0:AI1[3]])
  #pragma acc kernels
  ...
  for(int i=0;i<no_of_nodes;i++){
	...
	do
	{
		...
		h_updating_graph_mask_gpu[:no_of_nodes])
		{
  		...
  		long long int AI2[4];
  		AI2[0] = no_of_nodes > 0;
  		AI2[1] = (AI2[0] ? no_of_nodes : 0);
  		AI2[2] = 4 * AI2[1];
  		AI2[3] = AI2[2] / 4;
  		#pragma acc data copy(h_graph_mask_gpu[0:AI2[3]],h_graph_visited_gpu[0:AI2[3]],h_updating_graph_mask_gpu[0:AI2[3]])
  		#pragma acc kernels
  		#pragma acc loop independent
							dummyMethod3();
	...
	do
	{
		...
		for(tid = 0; tid < no_of_nodes; tid++ )
		{
  		...
  		long long int AI3[4];
  		AI3[0] = no_of_nodes > 0;
  		AI3[1] = (AI3[0] ? no_of_nodes : 0);
  		AI3[2] = 4 * AI3[1];
  		AI3[3] = AI3[2] / 4;
  		#pragma acc data copy(h_graph_mask[0:AI3[3]],h_updating_graph_mask[0:AI3[3]],h_graph_visited[0:AI3[3]])
  		#pragma acc kernels
  		#pragma acc loop independent
							dummyMethod3();
  		for(int tid=0; tid< no_of_nodes ; tid++ )

...
void init(DATA_TYPE * A, DATA_TYPE * B) {
	...
	#pragma acc data copy(B[0:8192],A[0:8192])
	#pragma acc kernels
...
void vec_mult(DATA_TYPE* A, DATA_TYPE* B, DATA_TYPE* C) {
	...
	#pragma acc data copy(C[0:8192],B[0:8192],A[0:8192])
	#pragma acc kernels
	#pragma acc loop independent
	dummyMethod3();
	for (i=0; i<N; i++)
...
void vec_mult_OMP(DATA_TYPE* A, DATA_TYPE* B, DATA_TYPE* C) {
	...
	#pragma omp target data map(to: A[:N], B[:N]) map(from: C[:N]) device(GPU_DEVICE)
	{
		...
		#pragma acc data copy(A[0:8192],B[0:8192],C[0:8192])
		#pragma acc kernels
		#pragma acc loop independent
		for (i=0; i<N; i++)
			C[i] = A[i] * B[i];
...
void compareResults(DATA_TYPE* B, DATA_TYPE* B_GPU)
{
  ...
  // Compare B and B_GPU
  #pragma acc data copy(B[0:8192],B_GPU[0:8192])
  #pragma acc kernels
  #pragma acc loop independent
			dummyMethod3();
  for (i=0; i < N; i++) 

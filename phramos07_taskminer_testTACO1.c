...
void corr(float *A, float *MEAN, float *STDEV, int m, int n) {
  ...
  #pragma omp target data map(to: A[0:AI1[8]]) map(tofrom: MEAN[0:AI1[13]]) if(!RST_AI1)
dummyMethod1();
...
dummyMethod2();
  ...
  char RST_AI2 = 0;
  RST_AI2 |= !((A + 0 > MEAN + AI2[13])
  || (MEAN + 0 > A + AI2[8]));
  RST_AI2 |= !((A + 0 > STDEV + AI2[13])
  || (STDEV + 0 > A + AI2[8]));
  RST_AI2 |= !((MEAN + 0 > STDEV + AI2[13])
  || (STDEV + 0 > MEAN + AI2[13]));
  #pragma omp target device (GPU_DEVICE) if(!RST_AI2)
  #pragma omp target data map(to: A[0:AI2[8]],MEAN[0:AI2[13]]) map(tofrom: STDEV[0:AI2[13]]) if(!RST_AI2)
			dummyMethod1();
  #pragma omp parallel for
  for (int i = 0; i < m; i++) {
    for (int j = 0; j < n; j++)

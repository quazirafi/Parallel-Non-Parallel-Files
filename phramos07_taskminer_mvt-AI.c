...
void init_array(DATA_TYPE* A, DATA_TYPE* x1, DATA_TYPE* x2, DATA_TYPE* y1, DATA_TYPE* y2, DATA_TYPE* x1_gpu, DATA_TYPE* x2_gpu)
{
  ...
  #pragma acc data copy(x2[0:4096],y1[0:4096],y2[0:4096],x1_gpu[0:4096],x2_gpu[0:4096],x1[0:4096],A[0:16781312])
  #pragma acc kernels
...
void runMvt(DATA_TYPE* a, DATA_TYPE* x1, DATA_TYPE* x2, DATA_TYPE* y1, DATA_TYPE* y2)
{
  ...
  #pragma acc data copy(y1[0:4096],a[0:16781312],x1[0:4096])
  #pragma acc kernels
  #pragma acc loop independent
			dummyMethod3();
  for (i=0; i<N; i++) 
  ...
  #pragma acc data copy(y2[0:4096],a[0:16781312],x2[0:4096])
  #pragma acc kernels
  #pragma acc loop independent
			dummyMethod3();
  for (i=0; i<N; i++) 
...
void runMvt_OMP(DATA_TYPE* a, DATA_TYPE* x1, DATA_TYPE* x2, DATA_TYPE* y1, DATA_TYPE* y2)
{
  ...
  #pragma acc data copy(x1[0:4096],y1[0:4096],a[0:16781312])
  #pragma acc kernels
  #pragma acc loop independent
  for (i=0; i<N; i++) 
    {
  ...
  #pragma omp target map(to: a[:N*N], y2[:N]) map(tofrom: x2[:N])
			dummyMethod1();
  #pragma omp parallel for collapse(1)
  #pragma acc data copy(x2[0:4096],y2[0:4096],a[0:16781312])
  #pragma acc kernels
  #pragma acc loop independent
  for (i=0; i<N; i++) 
    {
...
void compareResults(DATA_TYPE* x1, DATA_TYPE* x1_outputFromGpu, DATA_TYPE* x2, DATA_TYPE* x2_outputFromGpu)
{
  ...
  #pragma acc data copy(x1[0:4096],x1_outputFromGpu[0:4096],x2[0:4096],x2_outputFromGpu[0:4096])
  #pragma acc kernels
  #pragma acc loop independent
			dummyMethod3();
  for (i=0; i<N; i++) 
